...
public class AudioService {
    ...
    public ApiResponse<PromptResponse> processAudioPrompt(String jwt,MultipartFile audioFile){
        ...
        try {
            entity = new HttpEntity<>(audioFile.getBytes(), headers);
        } catch (Exception e) {
            throw new IllegalArgumentException("Failed to read audio file: " + e.getMessage());
        }
        PromptResponse microserviceResponse = template.postForObject(audioUrl, entity, PromptResponse.class);
